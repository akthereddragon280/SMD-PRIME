 ...
 * ============================================================================
 * Architecture: Autonomous Edge Daemon for Proactive OAuth Token Minting
 * 
 * DESIGN SPECIFICATIONS:
 * 1. ZERO PLAYBACK RUNTIME AUTH:
 *    - Eliminates all real-time Google Auth API requests during video streaming.
 *    - Prevents Google OAuth rate-limiting ("Thundering Herd" bans) and DB pool exhaustion.
 * 2. AUTONOMOUS CRON DAEMON:
 *    - Triggered every 45 minutes via Cloudflare Scheduled Events (`0/45 * * * *`).
 *    - Can also be manually triggered via HTTP (`GET /trigger?key=ADMIN_SECRET`).
 * 3. NATIVE WEBCRYPTO RS256 JWT SIGNING:
 *    - Uses zero external dependencies. Relies strictly on standard Web Crypto API
 *      (`RSASSA-PKCS1-v1_5` with `SHA-256`) for signing Google OAuth JWT assertions.
 * 4. CLOUDFLARE KV TOKEN PERSISTENCE:
 *    - Writes fresh Google Drive access tokens directly into Cloudflare KV (`SA_TOKENS`).
 *    - Key format: `sa:${email}` and `sa_index:${index}`.
 *    - Expiration TTL: 3300 seconds (55 minutes, ensuring seamless overlap).
 * 5. FAULT-TOLERANT RETRY LOOP:
 *    - If an individual SA token generation fails, the error is logged asynchronously
 *      and the engine immediately continues processing the remaining SA pool.
 * ============================================================================
 */

// Embedded Vault Fallback Service Account Credentials
...
const TOKEN_EXPIRATION_TTL_SEC = 3300; // 55 minutes (gives 5-minute buffer before 1-hour OAuth expiry)
const GOOGLE_TOKEN_ENDPOINT = 'https://oauth2.googleapis.com/token';

/**
 * 1. Base64URL Encoder Helper
 */
...
/**
 * 2. ArrayBuffer to Base64URL Helper
 */
...
/**
 * 3. PKCS#8 Private Key Importer via WebCrypto API
 * Parses standard PEM private key block and imports as CryptoKey object for signing.
 */
...
/**
 * 4. RS256 JWT Assertion Generator
 * Construct JWT header & payload, sign via WebCrypto API, and format as JWS.
 */
...
/**
 * 5. Fetch Fresh Access Token from Google OAuth2 Server
 */
...
/**
 * 6. Resolve Active Service Accounts Pool
 * Fetches from Supabase DB (if configured in env) or falls back to env JSON / hardcoded pool.
 */
async function resolveServiceAccountPool(env) {
  // Option A: Parse JSON pool from env variable if available
  ...
  // Option B: Query Supabase DB for active Service Accounts if bindings exist
  if (env?.SUPABASE_URL && env?.SUPABASE_ANON_KEY) {
    try {
      ...
      console.warn('[Cron Engine] Supabase fetch warning, falling back to embedded vault:', dbErr.message);
    }
  }

  // Option C: Return embedded hardcoded vault pool
  return FALLBACK_SA_POOL;
}

/**
 * 7. Core Daemon Token Refresh Loop
 * Iterates through all active SAs, fetches fresh OAuth tokens, and writes to Cloudflare KV.
 */
async function executeTokenRefreshPipeline(env) {
  ...
  for (let idx = 0; idx < saPool.length; idx++) {
    ...
    try {
      ...
      // KV WRITE 1: Store token by SA Email
      await kv.put(`sa:${saEmail}`, accessToken, {
        expirationTtl: TOKEN_EXPIRATION_TTL_SEC
      });

      // KV WRITE 2: Store token by Index Key (e.g. sa_index:1) for fast index lookup
      await kv.put(`sa_index:${saIndex}`, accessToken, {
        expirationTtl: TOKEN_EXPIRATION_TTL_SEC
      });

      validEmails.push(saEmail);
      results.successfulCount++;
      results.accountsProcessed.push({
        index: saIndex,
        email: saEmail,
        status: 'SUCCESS',
        ttl: TOKEN_EXPIRATION_TTL_SEC
      });

      console.log(`[Cron Engine] Token refreshed for SA #${saIndex} (${saEmail}) -> KV Write OK.`);
    } catch (saErr) {
      results.failedCount++;
      results.accountsProcessed.push({
        index: saIndex,
      ...
      console.error(`[Cron Engine] Token generation failed for SA #${saIndex} (${saEmail}):`, saErr.message);
      // Fault tolerance: Continue processing remaining SAs without crashing daemon
    }
  }

  // KV WRITE 3: Store active SA metadata list into KV for stream worker load balancer
  if (validEmails.length > 0) {
    await kv.put('ACTIVE_SA_EMAILS', JSON.stringify(validEmails), {
      expirationTtl: TOKEN_EXPIRATION_TTL_SEC
    });
    await kv.put('ACTIVE_SA_COUNT', String(validEmails.length), {
      expirationTtl: TOKEN_EXPIRATION_TTL_SEC
    });
  }
...
/**
 * 8. CLOUDFLARE WORKER EXPORT HANDLERS
 */
export default {
  /**
   * Cloudflare Scheduled Event Handler (Triggered every 45 minutes via Cron)
   */
  async scheduled(event, env, ctx) {
    console.log(`[Cron Engine Daemon] Scheduled Event Triggered at ${new Date().toISOString()}`);
    ctx.waitUntil(executeTokenRefreshPipeline(env));
  },

  /**
   * HTTP Fetch Entrypoint (For Manual Triggering & Diagnostic Status Checks)
   */
